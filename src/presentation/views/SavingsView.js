...
export async function renderSavings(container, savingsRepo, txRepo) {
    ...
    function openAddModal() {
      ...
      `,
            ...
            onConfirm: async () => {
                ...
                const type = body.querySelector('#s-type').value;
                const balance = parseFloat(body.querySelector('#s-balance').value) || 0;
    ...
    function openAmountModal(sav, mode) {
        openModal({
      ...
      `,
            ...
            onConfirm: async () => {
                const body = getModalBody();
                const value = parseFloat(body.querySelector('#am-value').value);
                const err = validateAmount(value);
                if (err) {
                    showToast(err, 'error');
                    return;
                }
                if (mode === 'withdraw' && value > sav.balance) {
    ...
    function openTransferModal(from, all) {
      ...
      `,
            ...
            onConfirm: async () => {
                const body = getModalBody();
                const toId = body.querySelector('#tr-to').value;
                const value = parseFloat(body.querySelector('#tr-value').value);
                const desc = body.querySelector('#tr-desc').value.trim() || 'Transferência';
                const err = validateAmount(value);
                if (err) {
                    showToast(err, 'error');
                    return;
