...
export async function renderCreditCards(container, cardRepo, txRepo) {
    ...
    async function render() {
        ...
        for (const card of cards) {
            ...
            const cardTx = transactions.filter(t => t.type === 'expense' && t.paymentMethod === 'card' && t.cardId === card.id &&
      ...
      `;
            ...
            if (cardTx.length === 0) {
                ...
                cardTx.sort((a, b) => b.date.localeCompare(a.date)).forEach(t => {
                    txList.appendChild(renderTransactionCard(t, async (id) => { await deleteTransaction(txRepo, id); showToast('Removido', 'success'); await render(); }, () => { }));
                });
            }
    ...
    function openCardModal(existing) {
      ...
      `,
            ...
            onConfirm: async () => {
                ...
                const limit = parseFloat(body.querySelector('#c-limit').value);
                const closing = parseInt(body.querySelector('#c-closing').value);
                const due = parseInt(body.querySelector('#c-due').value);
                const nameErr = validateName(name);
                const clErr = validateDay(closing);
                const dueErr = validateDay(due);
                if (nameErr || clErr || dueErr) {
                    showToast(nameErr ?? clErr ?? dueErr ?? 'Erro', 'error');
                    return;
                }
                if (existing) {
