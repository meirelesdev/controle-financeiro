import { getSheetNames, readSheet, detectHeaderRow, importGeneric, parseApMrvSheet, } from '../../application/use-cases/data/ImportFromExcel';
import { showToast } from '../components/Toast';
import { formatCurrency } from '../utils/formatters';
import Chart from 'chart.js/auto';
let chartMrv = null;
export async function renderImport(container, txRepo) {
    container.innerHTML = `
  ...
  `;
    ...
    function loadSheet(buffer, sheetName) {
    ...
    function renderGenericMapping(buffer, sheetName, overrideHeaderRow) {
        const detectedRow = overrideHeaderRow ?? detectHeaderRow(buffer, sheetName);
        const rows = readSheet(buffer, sheetName, detectedRow);
        if (rows.length === 0) {
            showToast('Aba vazia ou cabeçalho não detectado', 'warning');
            return;
        }
    ...
    `;
        ...
        document.getElementById('btn-reload-header')?.addEventListener('click', () => {
            const val = parseInt(document.getElementById('header-row-input').value);
            if (!isNaN(val) && val >= 1) {
        ...
        document.getElementById('btn-import-generic')?.addEventListener('click', async () => {
            const mapping = {
                ...
                type: document.getElementById('map-type').value || undefined,
                category: document.getElementById('map-category').value || undefined,
            };
            if (!mapping.date || !mapping.description || !mapping.amount) {
                showToast('Mapeie as colunas obrigatórias', 'error');
                return;
            }
    ...
    function renderApMrv(buffer, sheetName) {
    ...
    async function renderApMrvChart(rows) {
        await new Promise(r => setTimeout(r, 0));
        const ctx = document.getElementById('chart-mrv')?.getContext('2d');
        if (!ctx)
            return;
        chartMrv?.destroy();
