...
import type { Savings, SavingsType } from '../../../domain/entities/Savings'
...
export async function updateSavingsBalance(
  ...
  if (!existing) throw new Error(`Cofrinho ${id} não encontrado`)
...
export async function transferBetweenSavings(
  ...
  if (!from) throw new Error(`Cofrinho de origem ${fromId} não encontrado`)
  if (!to)   throw new Error(`Cofrinho de destino ${toId} não encontrado`)
  if (from.balance < amount) throw new Error('Saldo insuficiente no cofrinho de origem')
