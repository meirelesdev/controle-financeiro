...
export interface ColumnMapping {
  date:        string  // coluna da data
  description: string  // coluna da descrição
  amount:      string  // coluna do valor
  type?:       string  // coluna do tipo (entrada/saída) — opcional
  category?:   string  // coluna da categoria — opcional
}
 ...
 */
export function detectHeaderRow(buffer: ArrayBuffer, sheetName: string): number {
  const wb    = XLSX.read(buffer, { type: 'array' })
  ...
  for (let i = 0; i < Math.min(rows.length, 15); i++) {
    ...
    if (stringCols.length >= 3) return i
  }
...
export function readSheet(
  ...
  if (allRows.length <= detectedRow) return []

  const headerCells = (allRows[detectedRow] as unknown[]).map((v) =>
    v != null && String(v).trim() !== '' ? String(v).trim() : `COL_${allRows[detectedRow].indexOf(v)}`
  )
  ...
  for (let i = detectedRow + 1; i < allRows.length; i++) {
    ...
    headerCells.forEach((col, idx) => {
      obj[col] = row[idx] ?? ''
      if (row[idx] != null && row[idx] !== '') hasData = true
    })
 ...
 */
export function parseApMrvSheet(buffer: ArrayBuffer, sheetName: string): ApMrvRow[] {
  const rows = readSheet(buffer, sheetName)
  const result: ApMrvRow[] = []
  ...
  for (const row of rows) {
    ...
    const findCol = (...terms: string[]): string | undefined =>
      Object.keys(row).find(k => terms.some(t => k.toLowerCase().includes(t)))
    ...
    if (amortizacao <= 0 && juros <= 0) continue
