/** Calcula em qual mês uma compra no cartão vai aparecer na fatura.
 *  Se dia(compra) <= closingDay → mês atual
 *  Se dia(compra) >  closingDay → mês seguinte
 */
export function getTransactionBillingMonth(date, card) {
    const d = new Date(date + 'T12:00:00');
    const day = d.getDate();
    if (day <= card.closingDay) {
        return { year: d.getFullYear(), month: d.getMonth() + 1 };
    }
    else {
        const next = new Date(d.getFullYear(), d.getMonth() + 1, 1);
        return { year: next.getFullYear(), month: next.getMonth() + 1 };
    }
}
/** Melhor dia para comprar no cartão sem cair na fatura atual.
 *  Um dia após o fechamento, dentro do mesmo mês.
 */
export function getBestPurchaseDay(card) {
    return (card.closingDay % 28) + 1;
}
/** Resumo financeiro de uma lista de transações (já filtradas por mês). */
export function computeMonthlySummary(transactions) {
    let totalIncome = 0;
    let totalExpense = 0;
    let saldoReal = 0;
    let saldoProjetado = 0;
    let pendingCount = 0;
    const byCategory = {};
    for (const t of transactions) {
        if (t.type === 'transfer')
            continue;
        const sign = t.type === 'income' ? 1 : -1;
        const value = t.amount * sign;
        saldoProjetado += value;
        if (t.status === 'confirmado') {
            saldoReal += value;
            if (t.type === 'income')
                totalIncome += t.amount;
            if (t.type === 'expense')
                totalExpense += t.amount;
        }
        else {
            pendingCount++;
        }
        if (t.type === 'expense') {
            byCategory[t.category] = (byCategory[t.category] ?? 0) + t.amount;
        }
    }
    return { totalIncome, totalExpense, saldoReal, saldoProjetado, byCategory, pendingCount };
}
/** Agrupa transações por mês para o gráfico de histórico. */
export function computeMonthlyHistory(transactions, months = 6) {
    const result = [];
    const now = new Date();
    for (let i = months - 1; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const y = d.getFullYear();
        const m = d.getMonth() + 1;
        const monthTx = transactions.filter(t => {
            const td = new Date(t.date + 'T12:00:00');
            return td.getFullYear() === y && (td.getMonth() + 1) === m && t.status === 'confirmado';
        });
        const totalIncome = monthTx.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
        const totalExpense = monthTx.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
        result.push({ year: y, month: m, totalIncome, totalExpense });
    }
    return result;
}
/** Calcula fatura atual do cartão baseado nas transações do mês de faturamento. */
export function computeCardBill(card, allTransactions, year, month) {
    return allTransactions
        .filter(t => {
        if (t.type !== 'expense' || t.paymentMethod !== 'card' || t.cardId !== card.id)
            return false;
        const billing = getTransactionBillingMonth(t.date, card);
        return billing.year === year && billing.month === month;
    })
        .reduce((sum, t) => sum + t.amount, 0);
}
